...
При виборі алгоритму програміст стикається з кількома протиріччями:

- **Простота** — алгоритм повинен бути зрозумілим, легко програмуватись і налагоджуватись.
- **Ефективність** — алгоритм має швидко виконуватись і раціонально використовувати ресурси комп’ютера.

У реальній практиці вибір залежить від мети:
- Якщо програма виконується лише кілька разів — важливіше, щоб її було просто написати.  
- Якщо програма виконується тисячі або мільйони разів — тоді ефективність виходить на перший план.

Тому часто спочатку створюють **простий прототип алгоритму**, щоб зрозуміти його поведінку, а вже потім оптимізують.


Існує два основних типи оцінки:

1. **Практична (емпірична)** — базується на вимірюванні часу виконання на реальному комп’ютері.  
   Вона залежить від:  
   - типу процесора,  
   - ефективності компілятора,  
   - особливостей операційної системи,  
   - майстерності програміста.

2. **Теоретична (асимптотична)** — оцінює алгоритм незалежно від конкретного обладнання, визначаючи **залежність часу виконання від розміру вхідних даних**.  
   Цей підхід є універсальним і використовується у більшості теоретичних досліджень.
...
Залежно від типу даних час роботи алгоритму може змінюватися.  
Розрізняють:
- **Найгірший випадок** — максимальний час виконання.  
  Використовується найчастіше, оскільки гарантує безпечну оцінку.
- **Середній випадок** — очікуваний час при випадковому наборі даних.  
  Використовується, коли можна математично описати розподіл вхідних даних.
- **Найкращий випадок** — мінімальний час (наприклад, коли дані вже відсортовані).

## Основні правила аналізу складності

1. **Прості оператори** (присвоєння, читання, запис) — мають складність $O(1)$.  
2. **Послідовність операторів:**  
   $O(1) + O(N) + O(N^2) = O(N^2)$ — береться найбільший порядок.
3. **Умовні оператори:**  
   $O(умова) + \max(O(гілка1), O(гілка2))$.
4. **Цикли:**  
   Якщо тіло циклу має складність $O(f(N))$, а цикл виконується $N$ разів — загальна складність $O(N \cdot f(N))$.
5. **Вкладені цикли:**  
   Якщо цикл у циклі — множимо кількість повторень: $O(N^2)$, $O(N^3)$ тощо.

Для практичного порівняння алгоритмів проводять **тестування на реальних даних**.  
Це дозволяє:
- виявити «вузькі місця» (bottlenecks);
- оцінити поведінку на великих обсягах даних;
- виявити неочевидні ефекти оптимізації компілятора.

> Тестові дані мають бути максимально наближені до реальних.  
> Інакше результати експериментів не матимуть цінності.

## Точність та стійкість алгоритмів

У чисельних розрахунках важливо не лише, **як швидко**, а й **наскільки точно** алгоритм працює.  
Деякі алгоритми накопичують похибки, що робить результати недостовірними.  
Тому при аналізі складності завжди враховують ще два параметри:
- **Точність** — наближеність результату до істинного.
- **Стійкість** — здатність алгоритму давати коректні результати при незначних змінах у вхідних даних.


## Теми для самостійного вивчення


1. Методи експериментального визначення складності алгоритмів.  
2. Порівняльний аналіз алгоритмів сортування за складністю.  
3. Принципи роботи алгоритмів пошуку у графах (DFS, BFS, Dijkstra).  
4. Оцінка ефективності рекурсивних алгоритмів.  
5. Використання хеш-функцій у комп’ютерній безпеці.

## Контрольні питання


1. Що таке асимптотична складність алгоритму?  
2. Які існують основні типи складності за Big O?  
3. У чому полягає компроміс між часом і пам’яттю?  
4. Як визначити складність вкладених циклів?  
5. Що означає середній і найгірший випадок виконання алгоритму?  
6. Чому важливо тестувати алгоритми на реальних даних?  
7. Що таке точність і стійкість алгоритму?