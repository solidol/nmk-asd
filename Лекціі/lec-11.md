...
Деякий ефект дає використання так званого „золотого перерізу”. Це число ![](img/11-003.png), що має властивість:

![](img/11-004.png)

Доданій корінь ![](img/11-005.png) і є золотим перерізом.

Згідно цього алгоритму відрізок `b` … `e` слід ділити не навпіл, як у бінарному алгоритмі, а на відрізки, пропорційні ![](img/11-003.png) та 1, в залежності від того, до якого краю ближче `key`. Замість оператора
...
char a[N]; у якому слід знайти заданий рядок символів: ![](img/11-006.png) довжиною `m`.
